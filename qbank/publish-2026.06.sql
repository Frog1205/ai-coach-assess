...
-- 1. 先去激活旧版本(qbank_one_active 约束:同时只能 1 个激活)
UPDATE qbank_versions SET is_active = false WHERE is_active = true;
...
-- 3. 给 accounts 加 signup_source 字段
ALTER TABLE accounts ADD COLUMN IF NOT EXISTS signup_source jsonb;
COMMENT ON COLUMN accounts.signup_source IS 'First-touch attribution: UTM/referrer/channel';
